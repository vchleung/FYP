...
%
% Create a receiver configuration structure for MCRoomSim named 'Receivers'.
%
...
%
% DESCRIPTION:
% Set up receivers for MCRoomSim. A receiver is characterised by its 
% directivity, position, etc. MCRoomSim can simulate any number of 
% receivers (up to the memory limit of your computer).
...
% |-----------------------------------------------------------------------|
% |    Fs     | Sample rate of the receiver          | Any real-positive  |
% |           | directional impulse responses.       | number             |
% |           | ('impulse' type only)                |            {48000} |
% |-----------|--------------------------------------|--------------------|
% | Response  | 2D or 3D matrix of directional gains | 2D or 3D matrix of |
% |           | or directional impulse responses.    | real numbers       |
% |           | (see remarks below)                  |               {[]} |
% |-----------|--------------------------------------|--------------------|
% | Direction | 2D matrix of directions for which    | 2D  matrix of real |
% |           | responses are defined  (degrees).    | numbers            |
% |           | (see remarks below)                  |               {[]} |
% |-----------|--------------------------------------|--------------------|
%
...
%   soundfield  * Receiver that models a 'SoundField' microphone.
...
%                 desired directions. 
%               * Can be multichannel.
%               * 'gain' receivers require a 'Response' matrix and a 
%                 'Direction' matrix to be defined.
%   impulse     * A custom receiver specified with user defined impulse 
%                 responses at desired directions. 
%               * Can be multichannel.
%               * 'impulse' receivers require a 'Response' matrix, a 
%                 'Direction' matrix and a sampling rate value 'Fs' 
%                  to be defined. 
% - NFComp:
%   The Near Field Compensation Filter aims to simulate the curvature of 
%   the wavefront and the pressure decrease related to the radial distance
%   to the source. The Near Field Compensation filter is only applied to
%   the direct sound, not the reflections (reflections are considered to 
%   be in the far field).
% - NFCLimit:
%   For increasing orders and lower kr, the magnitude of NFC filters
%   increases dramatically. NFCLimit defines a hard limit on the 
%   magnitude of the NFC filters. Set this value to 0 dB to have the 
...
% - Direction:
%   2 column matrix specifying the directions at which the custom responses
%   are defined. Column 1 is azimuth and column 2 is elevation (both in
%   degrees, using Matlab's spherical coordinate definition). The number 
%   of rows (directions) in this matrix must match the number of 
%   gains/impulse responses specified per channel in the Response matrix.
%
% EXAMPLES:
% - Create a receiver structure and setup a dipole receiver at position
%   [2,3,5] oriented parallel to the y axis:
