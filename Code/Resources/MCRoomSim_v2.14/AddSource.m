...
%
% Create a source configuration structure named 'Sources' for MCRoomSim.
%
% Sources = AddSource(Sources,'FieldName',FieldValue,...)
%
% Add another source to 'Sources'. If no fields are passed then a default 
% source is created.
%
% DESCRIPTION:
% Set up sources for MCRoomSim. Each source has various characteristics, 
% such as directivity, position, etc. MCRoomSim can simulate any number 
% of sources (up to the memory limit of your computer).
...
% |-----------------------------------------------------------------------|
% |    Fs     | Sample rate of the source            | Any real-positive  |
...
% - Direction:
%   2-column matrix specifying the directions in which the custom responses
...
%   gains/impulse responses specified in the Response matrix.
%
% EXAMPLES:
...
elseif any(strcmpi(Sources(n).Type,{'bandkhats','bassdrum','bassoon',...
    ...
    if ~all(Sources(n).RefAngle==[0,0])
        ...
        [minDst,refDir] = min(Dst) ; %#ok<ASGLU>
        
        % Frequency response magnitudes
        FrqRsp = abs(fft(ImpRsp.')) ; %#ok<NODEF>
        ...
        for I =  1 : size(impAng,1)
            [tmp,ImpRsp(:,I)] = rceps(ImpRsp(:,I)) ; %#ok<ASGLU>
        end
    ...
    % Assign responses, directions and sampling frequency
