function [center, U, obj_fcn] = wcfcmval(data, W, beta, C, idx_e, U, cluster_n, options)
%Validation of Weighted Contextual FCM Data set clustering using fuzzy c-means clustering.
%
%
...
% Main loop
for i = 1:max_iter,
	[U, center, obj_fcn(i)] = stepwcfcm(data, W, beta, C, U, idx_e, cluster_n, expo);
	if display, 
...
function [U_new, center, obj_fcn] = stepwcfcm(data, W, beta, C, U, idx_e, cluster_n, expo)
...
for N = 1:size(U,1)
   n = 1:size(mf,1); n(N)=[];
   c(N,:) = cellfun(@(x) sum(sum(mf(n,x))),C);
end
center = mf(:,idx_e)*(W(idx_e).*data(idx_e))./(mf(:,idx_e)*W(idx_e)*ones(1,size(data(idx_e),2))); %new center
dist = distfcm(center, data);       % fill the distance matrix
dist = dist.^2;
obj_fcn = sum(dist(:,idx_e).*mf(:,idx_e)*W(idx_e))+beta/2*sum(sum(mf(:,idx_e).*c(:,idx_e)));  % objective function
tmpp = [W W]';
tmp = (dist.*tmpp+beta.*c).^(-1/(expo-1));      % calculate new U, suppose expo != 1
