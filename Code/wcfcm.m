function [center, U, obj_fcn] = wcfcm(data, W, beta, C, cluster_n, options)
%Weighted Contextual FCM Data set clustering using fuzzy c-means clustering.
%
%
...
% Main loop
for i = 1:max_iter,
	[U, center, obj_fcn(i)] = stepwcfcm(data, W, beta, C, U, cluster_n, expo);
	if display, 
...
function [U_new, center, obj_fcn] = stepwcfcm(data, W, beta, C, U, cluster_n, expo)
...
dist = dist.^2;
obj_fcn = sum(dist.*mf*W)+beta/2*sum(sum(mf.*c));  % objective function
tmpp = [W W]';
