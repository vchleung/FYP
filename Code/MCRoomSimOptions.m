...
%
% DESCRIPTION:
% MCRoomSim requires a number of options which specify things such as which
% type(s) of simulations to run, what order, thresholds, etc.
...
% |          |                                       |     {DefaultValue} |
% |----------|---------------------------------------|--------------------|
% | SimDirect| Simulate direct component of room     | true or false      |
...
% |----------|---------------------------------------|--------------------|
%
% MCRoomSimOptions generates this structure given some field values.
%
% REMARKS:
...
% - SimDiff, SimSpec and SimDirect:
%   Please note that these options only affect whether or not the diffuse,
%   specular and direct part of the simulation are performed. When setting 
%   SimDiff to 'false' the specular part of the simulation is run in the 
%   exact same manner as if SimDiff was set to true. In particular, this 
%   implies that the amplitude of the specular reflections depends on the
%   wall scattering coefficient, because scattering causes acoustic energy
%   to "leak" to the diffuse model at each reflection.
%   In other words, the proper way to simulate a room in which only 
%   specular reflections exist is to set the scattering coefficient to 0.
%
% EXAMPLES:
...
Options.NumRays     = 5292;              
Options.ReflRate    = 24000;              
...
for I = 1 : 2 : length(varargin)-1
    ...
    switch varargin{I}
        ...
        otherwise
            ...
            error([varargin{I} ' is not a recognised option field']) ;
